...
function computeMonthStats(
  ...
  // 품목수: 그날 계획(items) 행 개수 = 현황 페이지의 '금일 품목수'(items.length) 와 동일 기준.
  //  같은 코드라도 행이 여러 개면 각각 카운트(엑셀 '품목수'와 일치). 코드 중복 합치지 않음.
  //  items 가 있는 날만 분모에 들어가므로 토요일 등 생산 없는 날은 자동 제외.
  const itemCountByDay: Record<string, number> = {};
  useItems.forEach((it) => {
    itemCountByDay[it.date] = (itemCountByDay[it.date] || 0) + 1;
  });
  const counts = Object.values(itemCountByDay);
    ...
    <div className="space-y-5">
      <div className="bg-white border rounded-lg p-4 flex items-center gap-4 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            <span className="w-2 h-2 rounded-full bg-white" /> 실온 입력
          </button>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card label="총 생산량" value={stats.total.toLocaleString()} unit="EA" color="indigo" sub="냉장 + 실온" />
        <Card label="냉장 생산량" value={stats.coldTotal.toLocaleString()} unit="EA" color="blue" />
        <Card label="실온 생산량" value={stats.ambientTotal.toLocaleString()} unit="EA" color="orange" />
        <Card label="작업일 수" value={stats.daysWorked.toString()} unit="일" color="green" />
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card label="일평균 생산량" value={Math.round(stats.avgPerDay).toLocaleString()} unit="EA" color="purple" sub="냉장 + 실온" />
      ...
      {noData ? (
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-center gap-6 mt-4 text-sm text-gray-700 flex-wrap">
            ...
            <span className="flex items-center gap-2">
              <span className="w-4 h-4 inline-block rounded-sm" style={{ backgroundColor: '#ea580c' }} /> 실온
            </span>
            <span className="flex items-center gap-2">
...
function PrevMonthCompare({
  ...
  const rows: { label: string; cur: number; prv: number; unit: string; bold?: boolean; pp?: boolean; inverse?: boolean }[] = mode === 'full'
    ? [
        ...
        { label: '실온 생산량', cur: current.ambientTotal, prv: prev?.ambientTotal || 0, unit: 'EA' },
