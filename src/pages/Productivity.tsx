  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      ...
      {data.map((d, i) => d.total > 0 && (
        <text key={'tlab' + i} x={padL + i * bandW + bandW / 2} y={Math.max(padT + 10, yFor(d.total) - 10)} fontSize={12} textAnchor="middle" fill={TOTAL_COLOR} fontWeight="bold" stroke="white" strokeWidth={1.5} paintOrder="stroke">{d.total}</text>
      ))}
    </svg>
  );
}
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      ...
      {rows.map((r, i) => {
        ...
        return (
          <g key={'lab' + i}>
            <text x={xBase + barW / 2} y={barLabelY(r.a.total)} fontSize={11} textAnchor="middle" fill="#1e3a8a" fontWeight="bold" stroke="white" strokeWidth={1.5} paintOrder="stroke">{r.a.total || ''}</text>
            <text x={xBase + barW + barW / 2} y={barLabelY(r.b.total)} fontSize={11} textAnchor="middle" fill="#7f1d1d" fontWeight="bold" stroke="white" strokeWidth={1.5} paintOrder="stroke">{r.b.total || ''}</text>
          </g>
        );
      })}
