...
const KIND_LABEL: Record<string, { t: string; cls: string }> = {
  ...
  master:   { t: 'BOM 밖',    cls: 'bg-orange-100 text-orange-800' },
...
export default function DevRecipeImport() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setBom(b); setMaster([...useCount.values()].sort((a, x) => x.uses - a.uses));
  ...
  return (
    <div className="space-y-3">
      <div className="bg-indigo-50 border border-indigo-200 rounded p-2.5 text-xs text-indigo-900">
        ...
        · 애매한 매칭은 <b>확정하지 않고 후보만</b> 보여줍니다. 드롭다운에서 직접 고르세요.
      </div>
      ...
      <div className="border rounded p-2.5 space-y-2">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <div className="text-[11px] text-gray-500">
            {loading ? '기존 BOM 읽는 중...'
              : loaded ? <>참조 BOM <b>{bom.size}</b>개 제품 · 원재료 마스터 <b>{master.length}</b>종
                            ...
                            <tr key={i} className={!e.code ? 'bg-rose-50' : e.manual ? 'bg-blue-50/50' : ''}>
                              ...
                              <td className="px-2 py-1.5">
                                ...
                                >
                                  ...
                                  {/* 후보에 없는 것도 고를 수 있게 전체 마스터 */}
                                  <optgroup label="── 전체 원재료 ──">
                                    {master.filter((m) => !cands.some((c) => c.code === m.code)).slice(0, 300).map((m) => (
                                      <option key={m.code} value={`${m.code}||${m.name}`}>{m.name}</option>
                                    ))}
                                  </optgroup>
                                </select>
                              </td>
